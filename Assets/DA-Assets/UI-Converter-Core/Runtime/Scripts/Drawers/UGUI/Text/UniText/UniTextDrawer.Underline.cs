...
namespace DA_Assets.UCC.Drawers.CanvasDrawers
{
    ...
    {
#if UNITEXT
        private static void PopulateUnderline(UniText text, Node fobject)
        {
            ...
            if (!hasOverrides)
            {
                if (defaultStyle.TextDecoration == TextDecoration.UNDERLINE)
                    RegisterRangeRule(text, new UnderlineModifier())
            ...
            RegisterIfHasData(text, new UnderlineModifier(), data);
        }
#endif
    }
}
