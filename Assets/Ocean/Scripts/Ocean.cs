...
public class Ocean : MonoBehaviour {
	...
	static bool ShouldAssignOceanShader(Shader current)
	{
		if (current == null) return true;
		string n = current.name;
		if (string.IsNullOrEmpty(n)) return true;
		if (n.IndexOf("InternalError", System.StringComparison.Ordinal) >= 0) return true;
		if (n.StartsWith("Mobile/Ocean", System.StringComparison.Ordinal)) return true;
		if (n.IndexOf("OceanL", System.StringComparison.Ordinal) >= 0) return true;
		// Keep Universal / Shader Graph / other externally injected shaders.
		return false;
	}

	public void matSetVars() {
		// Do not stomp an externally injected URP shader with Mobile/Ocean (magenta under URP).
		// OceanNextGenAdapter sets material + oceanShader to URP Lit; only reassign when the
		// current shader is still a Built-in Ocean / error shader (or missing).
		if (material != null && oceanShader != null && ShouldAssignOceanShader(material.shader)) {
		...
		for(int i=0; i<3; i++) {
			if(mat[i]!= null) {
				...
				if (mat[i].HasProperty("_FoamFactor")) mat[i].SetFloat("_FoamFactor", foamFactor);
				if (mat[i].HasProperty("_FoamSize")) mat[i].SetFloat ("_FoamSize", foamUV);
				if (mat[i].HasProperty("_Size")) mat[i].SetFloat ("_Size", 0.015625f * bumpUV);
				if (mat[i].HasProperty("_WaterLod1Alpha")) mat[i].SetFloat("_WaterLod1Alpha", shaderAlpha);
				if (mat[i].HasProperty("_Specularity")) mat[i].SetFloat("_Specularity", specularity);
				if (mat[i].HasProperty("_SpecPower")) mat[i].SetFloat("_SpecPower", specPower);
				if (mat[i].HasProperty("_ShoreDistance")) mat[i].SetFloat("_ShoreDistance", shoreDistance);
		...
		// Ocean Built-in keywords are meaningless on URP Lit fallbacks — skip.
		if (IsUrpLitMaterial(mat != null && mat.Length > 0 ? mat[0] : material))
		{
			return;
		}
	...
	static bool IsUrpLitMaterial(Material m)
	{
		return m != null && m.shader != null
			&& m.shader.name.IndexOf("Universal Render Pipeline", System.StringComparison.Ordinal) >= 0;
	}
