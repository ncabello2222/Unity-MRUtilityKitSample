			...
			if (preset.Length > 0) {
				using (BinaryWriter swr = new BinaryWriter(File.Open(preset, FileMode.Create))) {
					...
					if(rrig != null) {
						...
						swr.Write((float)rrig.linearDamping);//float
						swr.Write((float)rrig.angularDamping);//float
					...
					if(File.Exists(preset)) {
						using (BinaryReader br = new BinaryReader(File.Open(preset, FileMode.Open))){
							...
							if(hasrigidbody) {
								...
								if(rrig != null) {
									rrig.mass = mass; rrig.linearDamping = drag; rrig.angularDamping = andrag; rrig.useGravity = usegrav;
